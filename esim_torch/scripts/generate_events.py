...
def process_dir(outdir, indir, args):
    ...
    for image_file, timestamp_ns in zip(image_files, timestamps_ns):
        ...
        # for iter in range(len(sub_events['t'])):
        #     temp = "%.8f %d %d %d"%(sub_events['t'][iter]/10e8,sub_events['x'][iter],sub_events['y'][iter],sub_events['p'][iter])
        #     temp_list = temp.split()
        #     if temp_list[3] == '-1':
        #         temp_list[3] = 0
        #     event_dict.append(temp_list)
