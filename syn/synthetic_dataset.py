...
def draw_ellipses(img, nb_ellipses=5):
...
def move_star(img,point_list,col_list,thickness_list):
    ...
    for iter in range(len(col_list)):
        point = point_list[iter+1]+moving_vec
        cv.line(img, center, point, col_list[iter],thickness_list[iter])
...
#一次画两张，不然太麻烦了
def draw_checkerboard_twice(img,img1, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)):
        ...
        for j in range(cols):
            ...
            cv.fillConvexPoly(img, np.array([(warped_points[i * (cols + 1) + j, 0],
                                            ...
                                            warped_points[(i + 1)
                                                            * (cols + 1) + j, 1])]),
                            col)
            cv.fillConvexPoly(img1, np.array([(warped_points1[i * (cols + 1) + j, 0],
    ...
    for _ in range(nb_rows):
        ...
        cv.line(img, (warped_points[row_idx * (cols + 1) + col_idx1, 0],
                ...
                warped_points[row_idx * (cols + 1) + col_idx2, 1]),
                col, thickness)
        cv.line(img1, (warped_points1[row_idx * (cols + 1) + col_idx1, 0],
            warped_points1[row_idx * (cols + 1) + col_idx1, 1]),
        (warped_points1[row_idx * (cols + 1) + col_idx2, 0],
        warped_points1[row_idx * (cols + 1) + col_idx2, 1]),
        col, thickness)
    for _ in range(nb_cols):
        col_idx = random_state.randint(cols + 1)
        row_idx1 = random_state.randint(rows + 1)
        ...
        cv.line(img, (warped_points[row_idx1 * (cols + 1) + col_idx, 0],
                ...
                warped_points[row_idx2 * (cols + 1) + col_idx, 1]),
                col, thickness)
        cv.line(img1, (warped_points1[row_idx1 * (cols + 1) + col_idx, 0],
                    warped_points1[row_idx1 * (cols + 1) + col_idx, 1]),
                (warped_points1[row_idx2 * (cols + 1) + col_idx, 0],
                warped_points1[row_idx2 * (cols + 1) + col_idx, 1]),
                col, thickness)

    # Keep only the points inside the image
    points = warped_points
    points1 = warped_points1
    # points = keep_points_inside(warped_points, img.shape[:2])
    # points1 = keep_points_inside(warped_points1, img.shape[:2])  
    return [points,points1]

def draw_stripes_twice(img,img1, max_nb_cols=13, min_width_ratio=0.04,
                 transform_params=(0.05, 0.15)):
    """ Draw stripes in a distorted rectangle and output the interest points
    Parameters:
      max_nb_cols: maximal number of stripes to be drawn
    ...
    for i in range(col):
        ...
        cv.fillConvexPoly(img, np.array([(warped_points[i, 0],
                                         ...
                                         (warped_points[i+col+1, 0],
                                          warped_points[i+col+1, 1])]),
                          color)
        cv.fillConvexPoly(img1, np.array([(warped_points1[i, 0],
                                          warped_points1[i, 1]),
                                         (warped_points1[i+1, 0],
                                          warped_points1[i+1, 1]),
                                         (warped_points1[i+col+2, 0],
                                          warped_points1[i+col+2, 1]),
                                         (warped_points1[i+col+1, 0],
                                          warped_points1[i+col+1, 1])]),
                          color)
    # Draw lines on the boundaries of the stripes at random
    nb_rows = random_state.randint(2, 5)
    nb_cols = random_state.randint(2, col + 2)
    ...
    for _ in range(nb_rows):
        ...
        cv.line(img, (warped_points[row_idx + col_idx1, 0],
                ...
                (warped_points[row_idx + col_idx2, 0],
                 warped_points[row_idx + col_idx2, 1]),
                color, thickness)
        cv.line(img1, (warped_points1[row_idx + col_idx1, 0],
                      warped_points1[row_idx + col_idx1, 1]),
                (warped_points1[row_idx + col_idx2, 0],
                 warped_points1[row_idx + col_idx2, 1]),
                color, thickness)
    for _ in range(nb_cols):
        col_idx = random_state.randint(col + 1)
        color = get_random_color(background_color)
        cv.line(img, (warped_points[col_idx, 0],
                ...
                (warped_points[col_idx + col + 1, 0],
                 warped_points[col_idx + col + 1, 1]),
                color, thickness)
        cv.line(img1, (warped_points1[col_idx, 0],
                      warped_points1[col_idx, 1]),
                (warped_points1[col_idx + col + 1, 0],
                 warped_points1[col_idx + col + 1, 1]),
                color, thickness)                
...
def draw_cube_twice(img,img1, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
              scale_interval=(0.4, 0.6), trans_interval=(0.5, 0.2)):
    """ Draw a 2D projection of a cube and output the corners that are visible
    Parameters:
      min_size_ratio: min(img.shape) * min_size_ratio is the smallest achievable
    ...
    for i in [0, 1, 2]:
        cv.fillPoly(img, [cube[faces[i]].reshape((-1, 1, 2))],
                    col_face)
        cv.fillPoly(img1, [cube1[faces[i]].reshape((-1, 1, 2))],
                    col_face)                    
    thickness = random_state.randint(min_dim * 0.01, min_dim * 0.015)
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
            ...
            cv.line(img, (cube[faces[i][j], 0], cube[faces[i][j], 1]),
                    (cube[faces[i][(j + 1) % 4], 0], cube[faces[i][(j + 1) % 4], 1]),
                    col_edge, thickness)
            cv.line(img1, (cube1[faces[i][j], 0], cube1[faces[i][j], 1]),
                    (cube1[faces[i][(j + 1) % 4], 0], cube1[faces[i][(j + 1) % 4], 1]),
                    col_edge, thickness)
...
def draw_polygon_test(img, max_sides=8):
    ...
    num_corners = points.shape[0]
    if num_corners < 3:  # not enough corners
        return draw_polygon_test(img, max_sides)
...
#生成纯背景
def generate_pure_background(size=(960, 1280)):
    """ Generate a customized background image
    Parameters:
      size: size of the image
    """
